...
def destination(request, pk):
    ...
    # send sms to driver phone number informing about all those to get out
    if tickets:
        driver_phone_number = tickets[0].driver_phone_number
        driver_message = f"Hello Driver, bellow passengers are to get off :\n{[ticket.name for ticket in tickets]}"
        # format driver_message
        driver_message = driver_message.replace("[", "").replace("]", "").replace(
            "'", ""
        ).replace(",", "\n")
        send_sms(driver_phone_number,driver_message )
...
def send_custom_message(request):
    ...
    try:
        ...
        return Response(
            {"message": "Please provide message_content"},
...
def get_tickets(request):
    try:
        ...
        return Response(
            {"message": "You have no trips"}, status=status.HTTP_404_NOT_FOUND
        )
    ...
    for destination in destinations:
        response[destination.name] = TicketSerializer(
...
def get_destinations(request):
    ...
    return Response(
        DestinationSerializer(destinations, many=True).data, status=status.HTTP_200_OK
    )
